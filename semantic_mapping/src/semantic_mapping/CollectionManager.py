...
class TypesCollection:
    ...
    def __init__(self, 
        base_ros_type:type, 
        input_parent: Optional[type]=None,
        input_response: Optional[type]=None,
        query_parent: Optional[type]=None, 
        query_response: Optional[type]=None,
        input_array_parent: Optional[type]=None,
        input_array_response: Optional[type]=None):
        ...
        self.query_parent = query_parent;
        ...
        self.input_array_parent = input_array_parent;
        self.input_array_response = input_array_response;    
...
class CollectionManager:
    ...
    def __init__(self, 
        ...
        visualisation_manager: Optional[visualisation.RvizVisualisationManager]=None,
        ...
        self.metadata_latent_adding: Optional[dict] = None;
        self.metadata_latent_querying: Optional[dict] = None;

        self.setupServices();
        

    def addItemToCollectionDict(self, adding_dict:dict) -> Optional[ObjectId]:
        """
    ...
    def addItemToCollection(self, adding) -> Optional[ObjectId]:
        """
        ...
        if len(utils.getUIDObj(adding)) > 0:
            ...
            uid = ObjectId(utils.getUIDObj(adding));
    ...
    def rosPushToCollectionArr(self, pushing_arr):
        ...
        for element in pushing_array:
            uid = self.addItemToCollection(element)
            uid_outputs.append("" if uid is None else str(uid));

        response = self.types.input_array_response(); #type: ignore
    ...
    def updateEntry(self, uid: ObjectId, update_to: dict, increment: Optional[dict]=None):
        """
    ...
    def rosQueryEntrypoint(self, ros_query):    # -> self.types.query_response
        ...
        if (HEADER_ID in ros_query_dict) and (UID_ENTRY in ros_query_dict[HEADER_ID]):
            ros_query_dict[utils.PYMONGO_ID_SPECIFIER] = ObjectId(ros_query_dict[HEADER_ID][UID_ENTRY]);
        ...
        ros_response = self.types.query_response(); #type: ignore
