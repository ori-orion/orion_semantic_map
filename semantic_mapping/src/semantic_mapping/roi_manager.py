...
class SOMAROIManager():
    ...
    def _init_menu(self):
        ...
        save_entry = self.menu_handler.insert("Save all ROIs", callback=self._save_cb)
    ...
    def _retrieve_objects(self):
        ...
        objs = self._msg_store.query(SOMROIObject._type)
        #print objs
        max_id = 0
        max_roi_id = 0
        for o,om in objs:
...
##            print self._soma_obj_roi_ids
    ...
    def add_object(self, soma_type, pose, roi_id=None):
        ...
        if roi_id == None:
            ...
            for key in self._soma_obj_pose:
                type = self._soma_obj_type[key]
                type_count[type] += 1

            if type_count[soma_type] > 0:
                roi_name = soma_type + str(type_count[type])
            else:
                roi_name = soma_type
            ...
            res = self._msg_store.query(SOMROIObject._type,message_query={'id':str(roi_id)})
