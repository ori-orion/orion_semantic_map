...
class SOMDataManager():
    def __init__(self, args):
        ...
        self.load_ont(args.ont_filename)
        self.load_rois(args.rois_filename)
        self.load_priors(args.priors_filename)
        self.init_services()
        print("Semantic database services initialised.")
        rospy.spin()

    def load_ont(self, file):
        self._ontology = Ontology(file)

    def load_priors(self, file):
        dirname = os.path.dirname(__file__)
        prior_path = os.path.join(dirname, "../..", "config", args.priors_filename)
        self._priors = read_prior_csv(prior_path)
    ...
    def handle_observe_request(self, req):
        ...
        res, id, obj = make_observation(
                            obs, self._rois, self._object_store,
                            self._observation_store, self._priors)
    ...
    def handle_delete_request(self,req):
        obj_id = req.obj_id
        response = self._observation_store.query(
                        SOMObject._type,message_query={"obj_id": obj_id})
